...
class ResultTable:
	...
	def startTableMouseAction(self):
		...
		item.setText(_translate("MainWindow", "Mouse Action"))
		item = self.avert_result_table.horizontalHeaderItem(3)
		item.setText(_translate("MainWindow", "Mouse Action Value"))
		item = self.avert_result_table.horizontalHeaderItem(4)
		item.setText(_translate("MainWindow", "Active Window"))
		item = self.avert_result_table.horizontalHeaderItem(5)
		item.setText(_translate("MainWindow", "Window Focus"))
		# __sortingEnabled = self.avert_result_table.isSortingEnabled()
		# self.avert_result_table.setSortingEnabled(False)
		# item = self.avert_result_table.item(0, 1)
		# item.setText(_translate("MainWindow", "10-06-26 02:31:29"))
	...
	def populateTableMouseAction(self):
			...
			item = QtWidgets.QTableWidgetItem()
			self.avert_result_table.setItem(0, 5, item)
			item = self.avert_result_table.item(0, 0)
			item.setText(_translate("MainWindow", newAttain['timestamp']))
			item = self.avert_result_table.item(0, 1)
			item.setText(_translate("MainWindow", newAttain['button']))
			item = self.avert_result_table.item(0, 2)
			position = newAttain['data']
			if position['clicked']:
				item.setText(_translate("MainWindow", "Click"))
			else:
				item.setText(_translate("MainWindow", "Movement"))
			item = self.avert_result_table.item(0, 3)
			item.setText(_translate("MainWindow", position['position']))
			item = self.avert_result_table.item(0, 4)
